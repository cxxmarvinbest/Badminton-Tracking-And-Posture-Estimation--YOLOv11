...
class PlayerTracker:
    ...
    def save_trajectory_heatmap(self):
        """保存运动员轨迹热力图（包含完整羽毛球场地图）"""
        ...
        for track_id, points in self.transformed_points.items():
            if track_id in self.player_names and points:
                ...
                # 创建网格
                xgrid = np.linspace(0, court_width, 100)
                ygrid = np.linspace(0, court_length, 200)
                ...
                # plt.contour(Xgrid, Ygrid, Z.reshape(Xgrid.shape), colors='k', linewidths=0.1)
        ...
        # 添加标签
        plt.title('Player Movement Heatmap', pad=20)
